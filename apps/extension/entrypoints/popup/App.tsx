...
export function App() {
  ...
  const statusMessage =
    model.connection.phase === "checking"
      ? "Looking for the local helper."
      : model.connection.status.message;
  ...
  return (
    <main className="flex flex-col gap-4 p-4">
      <header className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-5 text-zinc-400">
            Runs an action you choose when a download finishes.
          </p>
        </div>
      </header>
      ...
      <section className="rounded-lg border border-amber-400/50 bg-amber-400/10 px-3 py-2" role="status">
        <p className="text-xs font-semibold tracking-[0.16em] text-amber-200">DRY RUN</p>
        <p className="mt-1 text-sm leading-5 text-amber-50">
          Sleep, shut down, and restart are simulated. Nothing will actually happen.
        </p>
      </section>
      ...
      {showSetup ? (
        <section className="rounded-lg border border-zinc-800 bg-zinc-900/80 p-3 text-sm text-zinc-300">
          ...
          <p className="mt-2 text-xs leading-5 text-zinc-500">
            Host name <span className="text-zinc-300">{NATIVE_HOST_NAME}</span>. Full steps are in
            docs/local-development.md. Extension ID <span className="text-zinc-300">{model.extensionId}</span>.
          </p>
        </section>
      ) : null}
    </main>
